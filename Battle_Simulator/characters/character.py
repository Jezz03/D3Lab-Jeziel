...
class Character:
    ...
    def move(self, e_center: (float, float), m_vector: (float, float)) -> bool:
        ...
        Vt = (Vg[0] + m_vector[0], Vg[1] + m_vector[1])
        mag2 = math.sqrt(Vt[0] ** 2 + Vt[1] ** 2)
    ...
    def receiveDamage(self, damage: int) -> None:
        self._health = self.health - (damage*(self._protection/10.0))
        if(self._health <= 0):
            self._alive = False

    def isInRange(self, enemy) -> bool:
        d = math.sqrt((self._x - enemy.x)**2 + (self._y - enemy.y)**2)
